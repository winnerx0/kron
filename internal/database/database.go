...
func (d *Database) Start() *gorm.DB {
	dbUrl := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=%s sslmode=none",
	...
	err = db.AutoMigrate(&domain.Job{}, &domain.Execution{}, &user.User{}, &refreshtoken.RefreshToken{})
