...
const METHOD_COLORS: Record<string, string> = {
  ...
  PATCH:  'text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40 border-purple-200 dark:border-purple-900',
  DELETE: 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border-red-200 dark:border-red-900',
}

function MethodBadge({ method }: { method: string }) {
  return (
    <span className={`inline-block font-mono text-[10px] font-semibold px-2 py-0.5 rounded border ${METHOD_COLORS[method] ?? 'text-muted-foreground bg-muted border-border'}`}>
      {method}
    </span>
  )
}
...
function SkeletonRow() {
  return (
    <tr className="border-b border-border">
      ...
      <td className="px-5 py-3.5"><div className="skeleton h-5 w-12 rounded" /></td>
      <td className="px-5 py-3.5"><div className="skeleton h-3.5 w-52" /></td>
      <td className="px-5 py-3.5"><div className="skeleton h-6 w-24 rounded-full" /></td>
      <td className="px-5 py-3.5"><div className="skeleton h-3.5 w-10 ml-auto" /></td>
    </tr>
  )
}
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage your scheduled HTTP tasks</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {['Name', 'Method', 'Endpoint', 'Schedule', ''].map(h => (
                <th key={h} className={`px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground ${h === '' ? 'text-right' : 'text-left'}`}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={5}>
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    <CalendarClock className="w-10 h-10 text-muted-foreground/30 mb-4" strokeWidth={1} />
                    <p className="text-sm font-medium mb-1">No jobs yet</p>
                    <p className="text-sm text-muted-foreground mb-5">Create your first scheduled job to get started</p>
                    <Button size="sm" className="gap-2 h-8 text-xs"
                      onClick={() => { setSelected(null); setShowDialog(true) }}>
              ...
              >
                <td className="px-5 py-3.5 font-medium whitespace-nowrap">{job.name}</td>
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setSelected(job); setShowDialog(true) }}
      ...
      <CreateJobDialog
        ...
        job={selected}
        onJobSaved={() => { setShowDialog(false); setSelected(null); load() }}
      />
    </div>
  )
}
